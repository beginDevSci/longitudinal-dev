...
timepoints: 3_5
summary: Examine coupled developmental change across two constructs using bivariate latent change score models that estimate cross-domain coupling parameters in ABCD data.
description: Examine coupled developmental change across two constructs using bivariate latent change score models that estimate cross-domain coupling parameters in ABCD data.
---
...
The bivariate LCSM produces two types of key parameters. **Within-domain parameters** (means and variances of initial status and change scores) describe the trajectory of each construct separately, mirroring the univariate LCSM results. **Cross-domain coupling parameters** (gamma_hw and gamma_wh) are the primary quantities of interest in the bivariate model.

The coupling parameter **gamma_hw** tests whether height level predicts subsequent weight change: a positive value means taller youth gain more weight in the next period. The coupling parameter **gamma_wh** tests the reverse direction: whether weight level predicts subsequent height change. These parameters can reveal asymmetric coupling — for example, height may drive weight change but not vice versa — providing insight into the temporal ordering of developmental processes.

The **cross-domain covariances** (between initial statuses and between change scores) capture concurrent associations: whether youth who start taller also start heavier, and whether those who grow more in height also gain more weight within the same period. These are distinct from the coupling parameters, which test predictive (lagged) relationships across time.
...
### Scatter plot of coupled changes
coupled_plot <- ggplot(df_wide, aes(x = height_change_12, y = weight_change_12)) +
  geom_point(alpha = 0.15, size = 0.8, color = "#4A90D9") +
  geom_smooth(method = "lm", se = TRUE, color = "#E74C3C", linewidth = 1) +
  labs(
    title = "Coupled Change: Height vs. Weight (Baseline to Year 2)",
    x = "Height Change (in)",
    y = "Weight Change (lbs)"
  ) +
  theme_minimal()
...
The scatter plot displays the association between observed height change and weight change from baseline to year 2. The regression line summarizes the direction and strength of the concurrent association. A positive slope indicates that youth who grew more in height also tended to gain more weight, consistent with coordinated physical development. Note that these are observed associations — the BLCSM coupling parameters provide the latent (measurement-error-adjusted) estimates of cross-domain predictive relationships that operate with a temporal lag, which is a stronger test of directional influence than concurrent correlation.
...
Important limitations include the assumption that coupling effects are constant across time periods (when equality constraints are imposed), that the model requires at least three time points to identify coupling parameters, and that the measurement model assumes no systematic method effects. Extensions include models with proportional effects (change depending on own prior level in addition to the other domain), time-varying coupling strengths, and three-process models that incorporate a third developmental domain.
