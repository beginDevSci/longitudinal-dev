...
pub fn parse_data_preparation_section(
    events: &[Event],
                ...
                match marker.as_str() {
                    "code" => {
                        if let Some(block) = parse_code_block(events, i, &title, warnings) {
                            content_blocks.push(JsonDataPrepBlock::Code(block));
                        }
                    ...
                    "output" => {
                        if let Some(block) = parse_output_block(events, i, warnings) {
                            content_blocks.push(JsonDataPrepBlock::Output(block));
                        }
                        i = skip_to_next_h2(events, i);
                        continue;
                    }
                    "note" => {
                        if let Some(block) = parse_note_block(events, i, &title, warnings) {
                            content_blocks.push(JsonDataPrepBlock::Note(block));
                        }
                        i = skip_to_next_h2(events, i);
                        continue;
                    }
    ...
    if content_blocks.is_empty() {
        warnings.push("Data Preparation v2 section has no content blocks".to_string());
    }

    Ok(JsonDataPrep { content_blocks })
}
